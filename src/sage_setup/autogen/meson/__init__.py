...
def run(folder: Path, output_dir: Path, dry_run=False, force=False):
    ...
    for subdir in subdirs:
        if subdir.name.startswith('_') or subdir.name.startswith('.'):
            continue
        subdir_distributions, subdir_has_cython = run(subdir, output_dir / subdir.name, dry_run=dry_run, force=force)
...
def generate_meson():
    ...
    run(folder, output_dir, dry_run=args.dry_run, force=args.force)
    return 0
