...
def _inc(a, b) -> bool:
    """
    Compare orthogonal pairs.

    This compares two orthogonal pairs by the inclusion
    relation on the first sets and reverse inclusion on the second sets.
...
def poset_of_orthogonal_pairs(G):
    ...
    """
    return Poset([list(orthogonal_pairs(G)), _inc])
...
def mops(G):
    ...
    pairs of sets `(X, Y)` containing the maximal elements in a poset
...
def is_left_modular(L, H=None, verbose=False) -> bool:
    ...
    Testing a lattice that is not left-modular ::
...
def is_trim(L):
    ...
    returns a tuple ``(True, maximal_chains)`` if ``L`` is a trim lattice,
    where ``maximal_chains`` is a list of maximal chains in ``L``;
    returns ``(False, False)`` if ``L`` is not a trim lattice.

    ``L`` is trim, if there exists a maximal chain of length `n+1` containing
    only left-modular elements, and there are exactly
    `n` join-irreducible elements and meet-irreducible elements.
    ...
    .. NOTE::
        ...
        This computes all maximal length chains of ``L``
...
def toggle(G, x):
    ...
    We take a graph, ``G`` and compute the independent poset
    on ``G`` and ``tog_3(G)`` ::
...
def toggles(G, s):
    ...
    toggles all elements `x` in graph `G` (i.e. reverses all edges
    incident to `x`) as they appear in the sequence `s` starting at index 0.

    EXAMPLES:

    Example of a usage of toggles. We take a graph, `G` and compute the
    independent poset on `G` and `tog_2(tog_4(G))` two different ways:
