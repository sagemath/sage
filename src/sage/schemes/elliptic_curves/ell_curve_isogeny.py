...
def _is_cyclic_kernel_polynomial(E, psi, psi_2tor=None):
    r"""
    Return whether the subgroup defined by ``psi`` is cyclic.

    This test is purely algebraic: it checks the odd part via
    ``is_kernel_polynomial`` and handles the 2-primary part from the
    gcd with the two-division polynomial.
    """
    if psi_2tor is None:
        psi_2tor = two_torsion_part(E, psi)
    n2 = psi_2tor.degree()
    psi_odd = psi // psi_2tor

    if n2 == 0:
        from .isogeny_small_degree import is_kernel_polynomial
        return is_kernel_polynomial(E, 2 * psi.degree() + 1, psi)

    if n2 == 1:
        if psi_odd.degree() == 0:
            return True
        from .isogeny_small_degree import is_kernel_polynomial
        return is_kernel_polynomial(E, 2 * psi_odd.degree() + 1, psi_odd)

    if n2 == 3:
        return psi_odd.degree() == 0
...
def compute_codomain_kohel(E, kernel):
    ...
    if psi_2tor.degree() != 0: # even degree case
        ...
        if psi_quo.degree() != 0 and not _is_cyclic_kernel_polynomial(E, psi, psi_2tor):
...
class EllipticCurveIsogeny(EllipticCurveHom):
    ...
    def __init_from_kernel_polynomial(self, kernel_polynomial):
        ...
        These examples inherently exercise this private function::
            ...
            sage: # mixed odd and 2-torsion factors can still define cyclic kernels
            sage: F = GF(419)
            sage: E = EllipticCurve(F, [1, 0])
            sage: R.<x> = F[]
            sage: poly = x^3 - 25*x + 1
            sage: E.isogeny(poly)
            Isogeny of degree 8
             from Elliptic Curve defined by y^2 = x^3 + x over Finite Field of size 419
               to Elliptic Curve defined by y^2 = x^3 + 91*x + 412 over Finite Field of size 419
        """
        ...
        if psi_G.degree() != 0: # even degree case
            ...
            if psi_quo.degree() != 0 and not _is_cyclic_kernel_polynomial(E, psi, psi_G):
                raise NotImplementedError("Kohel's algorithm currently only supports cyclic isogenies (except for [2])")
