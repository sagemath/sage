...
def _is_cyclic_kernel_polynomial(E, psi):
    r"""
    Return whether the subgroup defined by ``psi`` is cyclic.

    The check uses invariant factors of the subgroup generated by one
    lift of each root of ``psi``.
    """
    roots = psi.roots(multiplicities=False)
    try:
        lifts = [E.lift_x(root) for root in roots]
        invariants = AdditiveAbelianGroupWrapper.from_generators(lifts, universe=E).invariants()
    except (TypeError, ValueError):
        return False
    return len(invariants) <= 1
...
def compute_codomain_kohel(E, kernel):
    ...
    if psi_2tor.degree() != 0: # even degree case
        ...
        if psi_quo.degree() != 0 and not _is_cyclic_kernel_polynomial(E, psi):
