...
class EllipticCurveHom_fractional(EllipticCurveHom):
    ...
    def to_isogeny_chain(self):
        ...
        for l, e in self._phi.degree().factor():
            ...
            assert len(pts) == 2 or l == p
            if not pts:
                # supersingular, hence [p] is the only p^2-isogeny up to isomorphism
                continue
            if len(pts) == 1:
                # ordinary, hence [p] is Frobenius times its dual
                insep_phi = self._phi.inseparable_degree().valuation(l)
                # The geometric kernel of phi is generated by
                # l^insep_phi * pts[0]; division by l^a multiplies
                # that generator by l^a.
                ker.append(l**(insep_phi + self._d.valuation(l)) * pts[0])
                continue
