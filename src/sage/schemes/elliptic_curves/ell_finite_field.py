...
class EllipticCurve_finite_field(EllipticCurve_field, HyperellipticCurve_finite_field):
    ...
    def height_above_floor(self, ell, e):
        ...
        .. NOTE::
            ...
            For an ordinary `E/\F_q`, and a prime `\ell`, the height
            of the `\ell`-volcano containing `j(E)` is the `\ell`-adic
            valuation of the conductor the order generated by the
    ...
    def endomorphism_discriminant_from_class_number(self, h):
        ...
        number ``h`` the output is undefined.

        ALGORITHM:

        Compute the trace of Frobenius and hence the discriminant `D_0`
        and class number `h_0` of the maximal order containing the
        endomorphism order.  From the given value of `h`, which must be a
        multiple of `h_0`, compute the possible conductors, using
        :meth:`height_above_floor` for each prime `\ell` dividing the
        quotient `h/h_0`.  If exactly one conductor `f` remains, return
        `f^2D_0`, otherwise return `0`.
