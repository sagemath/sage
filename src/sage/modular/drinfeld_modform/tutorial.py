...
elliptic curves re replaced by Drinfeld modules. In short, there exists
...
To create the ring of type zero and rank `r` Drinfeld modular forms in
SageMath, one uses the class
...
This ring is generated by three elements: the coefficient forms
`g_1, g_2` and `g_3`. Recall that the weight of `g_i` is `q^i - 1`::
...
To access a single coefficient form, one use the method
:meth:`coefficient_form`::
