...
def canonical_parameters(group, level, weight, base_ring):
    ...
    elif arithgroup.isinstance(group, CongruenceSubgroupBase):
        if ( Integer(level) != group.level() ):
            raise ValueError("group.level() and level do not match.")
        # normalize the case of SL2Z
        if arithgroup.isinstance(group, SL2Z_class) or \
           arithgroup.isinstance(group, Gamma1_class) and group.level() == Integer(1):
...
def ModularForms(group=1,
    ...
    if isinstance(group, dirichlet.DirichletCharacter) \
           or arithgroup.isinstance(group, CongruenceSubgroupBase):
        level = group.level()
    else:
        level = group
