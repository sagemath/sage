...
def cographs(n, as_graph=True, immutable=False):
    ...
    This method implements the generator of all cographs of order `n` proposd
    in [JPD2018]_. The algorithm generates one by one every cotree with `n` nodes,
    and each cotree is generated by using its previous cotree. The time to
    construct the first cotree is `O(n)` and the time spent between two
    consecutive outputs is `O(n)`. Hence, the overall complexity of the algorithm
    is `O(n*Mn)`, where `n` is the number of nodes and `Mn` is the total number of
    cographs with `n` nodes (see :oeis:`A000084`).
