...
class AugmentedChowRingIdeal_fy(ChowRingIdeal):
    ...
    for a simple matroid `M` is defined as the ideal
    `(I_{FY}(M))` of the following polynomial ring

    .. MATH::

        R[y_{e_1}, \ldots, y_{e_n}, y_{F_1 \union e}, \ldots, y_{F_k \union e}],
    ...
    - `I_{FY}(M) is the ideal generated by  all quadratic forms 
      `y_{F_i \union e} y_{F_j \union e}`, where `F_i` and `F_j`
      are incomparable elements in the lattice of flats, `y_{i} y_{F \union e}`
      for every `i \in E` and `i \notin F`, linear forms

      .. MATH::

        y_i + \sum_{i \in F} y_{F \union e}
    ...
    Setting `x_F = y_{F \union e}` and using the last linear
