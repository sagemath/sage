...
cdef class Matrix_polynomial_dense(Matrix_generic_dense):
    ...
    def basis_completion(self, row_wise=True, algorithm="approximant"):
        ...
        column-wise completing a column basis if `row_wise` is False.

        For a more detailed description, consider the row-wise case (the
        column-wise case is the same up to matrix transposition). For an `m \times
        n` input matrix `A`, whose rank `r` is a priori unknown, this computes a
        matrix `C` of dimensions `(n-r) \times n` such that stacking both matrices
        as `[[A],[C]]` gives a matrix of maximal rank `n` and with the same
        nontrivial Smith factors as `A`. In particular, the rank of the input
        matrix may be recovered from the number of rows of `C`. As consequences:

        - If `B` is a basis of the module generated by the rows of `A` (for example
        `B = A` if `A` has full row rank), then `[[B],[C]]` is nonsingular, and
        its determinant is the product of the nonzero Smith factors of `A`.

        - For `A` with full row rank: if `A` has unimodular column bases, or
        equivalently if the rows of `A` generate all polynomial vectors in the
        rational row space of `A`, then `C` is a unimodular completion of `A`, that
        is, `det([[A],[C]]) = 1`. (In general this determinant could be another
        nonzero constant, but the present implementation guarantees it is 1.)
        ...
        - The approximant-based algorithm follows the ideas in [ZL2014]_ ,
          based on polynomial reversals combined with the computation of a
          minimal kernel basis and a minimal approximant basis.

        - The Smith form-based algorithm computes the Smith form of this matrix
          along with corresponding unimodular transformations, from which a
          completion is readily obtained.
