...
cdef class Matrix_polynomial_dense(Matrix_generic_dense):
    ...
    def expansion(self, degree=None):
        r"""
        Return the horizontal expansion of the matrix,
        where the ith block corresponds to the coefficients for x^i.
        
        INPUT:

        - ``degree`` -- integer, the degree bound on the matrix;
        ...
        EXAMPLES:
            ...
            [96 96  1  0  0  0]
            sage: M.expansion(3)
            [ 0  0  0 36 31  0  1  0  0  0  0  0]
            [13 57  0  3  1  0  0  0  0  0  0  0]
        ...
        # compute each coefficient matrix
        coeff_matrices = [[self.coefficient_matrix(i) for i in range(degree+1)]]
        return matrix.block(coeff_matrices,subdivide=False)