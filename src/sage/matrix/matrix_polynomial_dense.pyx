...
cdef class Matrix_polynomial_dense(Matrix_generic_dense):
    ...
    def minimal_relation_basis(self,
        ...
        method computes in ``shifts``-ordered weak Popov form. If
        ``normal_form`` is ``True``, then the output basis `P` is further
        normalized: it is in the canonical ``shifts``-Popov form.

        It is guaranteed that the degree of the output basis is at most the
        degree of determinant of ``mat``, independently of ``shifts``.
