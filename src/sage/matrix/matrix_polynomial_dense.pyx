...
cdef class Matrix_polynomial_dense(Matrix_generic_dense):
    ...
    def minimal_relation_basis(self,
        ...
        polynomial matrix modulo ``mat``.

        If we work row-wise (which is the default), if `F` is an `m \times n`
        polynomial matrix and `M` is a column reduced `n \times n` matrix, then
        a relation basis `P` for `F` modulo `M` is a polynomial matrix whose
        rows form a basis of the module of polynomial vectors `p` of size `m`
        such that `p F = 0 \bmod M`, meaning that `p F` belongs to the module
        generated by the rows of `M`.

        Such a basis `P` is an `m \times m` nonsingular matrix, which this
        method computes in ``shifts``-ordered weak Popov form. It is
        guaranteed that the degree of the output basis is at most the degree
        of determinant of ``mat``, independently of ``shifts``. If
        ``normal_form`` is ``True``, then this output basis `P` is further
        normalized: it is the canonical ``shifts``-Popov basis.

        Here are two special cases.

        - minimal approximant bases, for which `M` is a diagonal of powers of
          the variable (see :meth:`minimal_approximant_basis`, which may be
          called directly for better performance),
        - minimal interpolant bases, for which `M` is a diagonal of polynomials
          that split into linear factors.

        If ``row_wise`` is ``False``, then, similarly, the columns of `P` form
        a basis of the right-relations, i.e. of the module of column vectors
        `p` such that `F p` is in the column space of `M`.

        An error is raised if the input dimensions are not sound: if working
        row-wise (resp. column-wise), the matrix dimensions of ``mat`` must be
        the number of columns (resp. rows) of ``self``, while the length of
        ``shifts`` must be the number of rows (resp. columns) of ``self``.
        There is no check about the reducedness of ``mat`` or about the
        degree constraints concerning ``self`` and ``mat``.

        INPUT:

        - ``mat`` -- polynomial matrix
        ...
        - ``row_wise`` -- boolean (default: ``True``). If ``True`` then the
          ...
          ``self``; ``mat`` must be column reduced and its column degrees must
          strictly bound those of ``self`` entry-wise. Otherwise the output
          basis is column-wise and operates on the right of ``self``; ``mat``
          must be row reduced and its row degrees must strictly bound those of
          ``self`` entry-wise.
        ...
        same as an approximant basis::
            ...
            sage: mat = matrix.diagonal([x**4, x**3], sparse=False)
            sage: shifts = [-1, 2, 0]
            sage: F = matrix(pR, [[5*x^3 + 4*x^2 + 4*x + 6, 5*x^2 + 4*x + 1],
            ....:                 [        2*x^2 + 2*x + 3, 6*x^2 + 6*x + 3],
            ....:                 [4*x^3         +   x + 1, 4*x^2 + 2*x + 3]])
            sage: P_app = F.minimal_approximant_basis([4, 3], shifts, normal_form=True)
            sage: P_rel = F.minimal_relation_basis(mat, shifts, normal_form=True)
            sage: P_app == P_rel
            True

        If ``self`` is the identity matrix, then relation bases are simply
        matrices that are left-unimodularly equivalent to ``mat``::

            sage: # mat is both row and column reduced
            sage: mat = x**4 * 1 + matrix.random(pR, 3, 3, degree=3)
            sage: F = matrix.identity(pR, 3)  # cdeg(F) < cdeg(mat)
            sage: P = F.minimal_relation_basis(mat, shifts=[0,1,2], normal_form=True)
            sage: P == mat.popov_form(shifts=[0,1,2])
            True
        ...
        the uniform `[0,\ldots, 0]` shift::
            ...
            sage: P = F.minimal_relation_basis(mat, row_wise=False)
            sage: P.is_weak_popov(shifts=[0]*5, row_wise=False)
            True
            sage: Q,R = (F*P).left_quo_rem(mat)  # F*P = mat*Q + 0
            sage: R == 0
            True
