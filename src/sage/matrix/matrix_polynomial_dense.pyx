...
cdef class Matrix_polynomial_dense(Matrix_generic_dense):
    ...
    def basis_completion(self, row_wise=True, algorithm="approximant"):
        ...
        the same nontrivial Smith factors as `A`. In particular, the rank of
        ...
        In particular, for `A` with full row rank: if 
the rows `A` can be completed into a basis of `\Bold{K}[x]^{n}`
         (or equivalently, `A` has unimodular
        column bases, or also, if the rows of `A` generate all
        polynomial vectors in the rational row space of `A`), then 
        `C` provides such a completion. In this case, `[[A],[C]]` is
        unimodular: it is invertible over `\Bold{K}[x]`, and `det([[A],[C]])`
        is a nonzero element of the base field `\Bold{K}`.
    ...
    def is_basis_completion(self, mat, row_wise=True):
        ...
        Using the same examples as :meth:`basis_completion`::
            ...
            sage: ring.<x> = GF(7)[]
            sage: mat = matrix([[x*(x-1)*(x-2), (x-2)*(x-3)*(x-4), (x-4)*(x-5)*(x-6)]])
            sage: rcomp = mat.basis_completion()
            sage: rcomp.is_basis_completion(mat)
            True
            ...
            sage: rcomp.is_basis_completion(mat)
            True
            sage: (x * rcomp).is_basis_completion(mat)
            False
            sage: rcomp.is_basis_completion(mat, row_wise=False)
            False
            sage: ccomp = mat.basis_completion(row_wise=False)
            sage: ccomp.is_basis_completion(mat, row_wise=False)
            True
            sage: ccomp.is_basis_completion(mat, row_wise=True)
            False
            sage: ((x+2) * ccomp).is_basis_completion(mat, row_wise=False)
            False
