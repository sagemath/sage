...
class SimplicialComplex(Parent, GenericCellComplex):
    ...
    def core(self, is_mutable=True):
        ...
        The core of a simplicial complex `\Delta` is the full
        subcomplex of `\Delta`, generated by a set of vertices
        `\{v \mid \mathrm{star}_\Delta v\}`.
    ...
    def is_gorenstein(self, base_ring=QQ):
        ...
        A Cohen-Macaulay simplicial complex `\Delta` of dimension
        `n-1` with `m` vertices is called Gorenstein if the sum of
        bigraded Betti numbers `\sum_j \beta^{-(m-n), 2j}` is equal to 1.
        ...
        .. SEEALSO::
            ...
            :meth:`is_cohen_macaulay`,
            :meth:`bigraded_betti_number`.
        ...
        m = len(self.vertices())
        n = self.dimension() + 1;
        b = sum(self.bigraded_betti_number(-(m-n), 2*j, base_ring=base_ring) for j in range(m+1))
        return b == 1
    ...
    def is_gorenstein_star(self, base_ring=QQ):
        ...
        A simplicial complex `\Delta` is called Gorenstein`^*`
        if it is Gorenstein and `\Delta = \mathrm{core} \Delta`.
