...
:class:`~sage.sets.recursively_enumerated_set.RecursivelyEnumeratedSet_forest`, and uses its
...
Each running map reduce instance works on a
:class:`~sage.sets.recursively_enumerated_set.RecursivelyEnumeratedSet_forest`
...
- ``_results`` -- a :class:`~multiprocessing.SimpleQueue` where
...
- ``worker._todo`` -- a :class:`collections.deque` storing of nodes of the
  ...
  this queue.
- ``worker._request`` -- a :class:`~multiprocessing.SimpleQueue` storing
  steal request submitted to ``worker``
- ``worker._read_task``, ``worker._write_task`` -- a
  :func:`~multiprocessing.Pipe` used to transfer node during steal
