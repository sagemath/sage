cdef extern from "lcalc_sage.h":
    ...
    ctypedef struct c_Lfunction_I "L_function<int>":
        ...
        int (* compute_rank) ()
        double (* N) (double T)
        void (* find_zeros_v)(double T1, double T2, double stepsize,
                              doublevec result)
    ...
    ctypedef struct c_Lfunction_Zeta "L_function<int>":
        ...
        void (* find_zeros_v)(double T1, double T2, double stepsize,
                              doublevec result)
        int (*find_zeros)(long count, long start, double max_refine, int rank, const char* message_stamp, doublevec* result)
        void (*find_zeros_via_N)(long count, int do_negative, double max_refine,
                                 int rank, int test_explicit_formula,
                                 char *filename)  # puts result in filename
