...
class TikzPicture(Standalone):
    ...
    This is using tkz-graph tex library::
        ...
        sage: g = graphs.PetersenGraph()                                                # needs sage.graphs
        sage: s = latex(g)                                              # optional - latex, needs sage.graphs
        sage: t = TikzPicture(s, standalone_config=["border=4mm"],      # optional - latex, needs sage.graphs
        ....:                 usepackage=['tkz-graph'])
        sage: _ = t.pdf(view=False)  # long time (2s), optional - latex latex_package_tkz_graph, needs sage.graphs
    ...
    def from_dot_string(cls, dotdata, prog='dot'):
        ...
        ::
            ...
            sage: G = W.cayley_graph()                                                  # needs sage.combinat sage.graphs sage.groups
            sage: dotdata = G.graphviz_string()                                         # needs sage.combinat sage.graphs sage.groups
            sage: tikz = TikzPicture.from_dot_string(dotdata)   # long time (3s), optional - dot2tex graphviz
            sage: _ = tikz.pdf()                # not tested                            # needs sage.combinat sage.groups

        ::

            sage: # needs sage.combinat sage.graphs sage.groups
            sage: dotdata = G.graphviz_string(labels='latex')                           # needs sage.combinat sage.graphs sage.groups
            sage: tikz = TikzPicture.from_dot_string(dotdata)   # long time (3s), optional - dot2tex graphviz
            sage: _ = tikz.pdf()                # not tested                            # needs sage.combinat sage.groups
    ...
    def from_graph(cls, graph, merge_multiedges=True,
        ...
        Using ``merge_multiedges``::
            ...
            sage: alpha = var('alpha')                                                                                  # needs sage.symbolic
            sage: m = matrix(2, range(4)); m.set_immutable()                                                            # needs sage.modules sage.symbolic
            sage: G = DiGraph([(0,1,alpha), (0,1,0), (0,2,9), (0,2,m)], multiedges=True)            # needs sage.graphs sage.modules sage.symbolic
            sage: tikz = TikzPicture.from_graph(G, merge_multiedges=True)       # optional - dot2tex graphviz, needs sage.graphs sage.modules sage.symbolic
            sage: _ = tikz.pdf()                # not tested                            # needs sage.modules sage.symbolic
