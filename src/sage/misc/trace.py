...
def trace(code, preparse=True):
    ...
    sage subprocess that uses IPython::
        ...
        sage: s = pexpect.spawn('sage')                                                 # optional - pexpect
        sage: _ = s.sendline("trace('print(factor(10))'); print(3+97)")                 # optional - pexpect
        sage: _ = s.expect('ipdb>', timeout=90)                                         # optional - pexpect
        sage: _ = s.sendline("s"); _ = s.sendline("c")                                  # optional - pexpect
        sage: _ = s.expect('100', timeout=90)                                           # optional - pexpect

    Seeing the ipdb prompt and the 2 \* 5 in the output below is a
    strong indication that the trace command worked correctly::

        sage: print(s.before[s.before.find(b'--'):].decode())                           # optional - pexpect
        --...
        ...ipdb> c
        ...2 * 5...
