...
cpdef GenericBackend get_solver(constraint_generation = False, solver = None, base_ring = None):
    ...
    EXAMPLES::
        ...
        sage: p.base_ring()                                                           # optional - sage.rings.number_field
        Rational Field
        sage: p = get_solver(solver='Matrix', base_ring=RR); p                        # optional - sage.rings.number_field
    ...
    if solver == "Coin":
        ...
        from sage.numerical.backends.matrix_backend import MatrixBackend
        return MatrixBackend(base_ring=base_ring)

    else:
        raise ValueError("'solver' should be set to 'GLPK', 'GLPK/exact', 'Coin', 'CPLEX', 'CVXOPT', 'CVXPY', 'Gurobi', 'PPL', 'SCIP', 'InteractiveLP',  'Matrix', None (in which case the default one is used), or a callable.")
