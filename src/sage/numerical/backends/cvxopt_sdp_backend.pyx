...
cdef class CVXOPTSDPBackend(MatrixSDPBackend):
    ...
    def __init__(self, maximization=True, base_ring=None):
        ...
        EXAMPLES::
            ...
            sage: p = get_solver(solver = "CVXOPT")                                     # needs cvxopt
    ...
    cpdef int solve(self) except -1:
        ...
        EXAMPLES::
            ...
            sage: p = SemidefiniteProgram(solver = "cvxopt", maximization=False)        # needs cvxopt
            sage: x = p.new_variable()                                                  # needs cvxopt
            sage: p.set_objective(x[0] - x[1] + x[2])                                   # needs cvxopt
            ...
            sage: p.add_constraint(a1*x[0] + a3*x[2] <= a4)                             # needs cvxopt
            sage: p.add_constraint(b1*x[0] + b2*x[1] + b3*x[2] <= b4)                   # needs cvxopt
            sage: N(p.solve(), digits=4)                                                # needs cvxopt
            -3.225
            sage: p = SemidefiniteProgram(solver = "cvxopt", maximization=False)        # needs cvxopt
            sage: x = p.new_variable()                                                  # needs cvxopt
            sage: p.set_objective(x[0] - x[1] + x[2])                                   # needs cvxopt
            ...
            sage: b3 = matrix([[-5.,   2., -17.], [2.,  -6.,   8.], [-17.,  8., 6.]])
            sage: b4 = matrix([[14., 9., 40.], [9., 91., 10.], [40., 10., 15.]])
            sage: p.add_constraint(a1*x[0] + a2*x[1] + a3*x[2] <= a4)                   # needs cvxopt
            sage: p.add_constraint(b1*x[0] + b2*x[1] + b3*x[2] <= b4)                   # needs cvxopt
            sage: N(p.solve(), digits=4)                                                # needs cvxopt
            -3.154
    ...
    cpdef get_objective_value(self):
        ...
        EXAMPLES::
            ...
            sage: N(p.solve(), digits=4)                                                # needs cvxopt
            -3.154
            sage: N(p.get_backend().get_objective_value(), digits=4)                    # needs cvxopt
            -3.154
        """
    ...
    cpdef _get_answer(self):
        ...
        TESTS::
            ...
            sage: p = SemidefiniteProgram(maximization = False, solver='cvxopt')        # needs cvxopt
            sage: x = p.new_variable()                                                  # needs cvxopt
            sage: p.set_objective(x[0] - x[1])                                          # needs cvxopt
            ...
            sage: p.add_constraint(a1*x[0] + a2*x[1] <= a3)                             # needs cvxopt
            sage: p.add_constraint(b1*x[0] + b2*x[1] <= b3)                             # needs cvxopt
            sage: p.solve();                                     # tol 1e-08            # needs cvxopt
            -3.0
            sage: p.get_backend()._get_answer()                                         # needs cvxopt
            {...}
        """
        return self.answer
    ...
    cpdef get_variable_value(self, int variable):
        ...
        EXAMPLES::
            ...
            sage: N(p.solve(), digits=4)                                                # needs cvxopt
            -3.154
            sage: N(p.get_backend().get_variable_value(0), digits=3)                    # needs cvxopt
            -0.368
            sage: N(p.get_backend().get_variable_value(1), digits=4)                    # needs cvxopt
            1.898
            sage: N(p.get_backend().get_variable_value(2), digits=3)                    # needs cvxopt
            -0.888
        """
    ...
    cpdef dual_variable(self, int i, sparse=False):
        ...
        EXAMPLES::
            ...
            sage: p.solve()                                     # tol 1e-08             # needs cvxopt
            -3.0
            sage: B=p.get_backend()                                                     # needs cvxopt
            sage: x=p.get_values(x).values()                                            # needs cvxopt
            sage: -(a3*B.dual_variable(0)).trace()-(b3*B.dual_variable(1)).trace()       # tol 1e-07                    # needs cvxopt
            -3.0
            sage: g = sum((B.slack(j)*B.dual_variable(j)).trace() for j in range(2)); g  # tol 1.5e-08                  # needs cvxopt
            0.0


        TESTS::

            sage: B.dual_variable(7)                                                    # needs cvxopt
            Traceback (most recent call last):
            ...
            IndexError: list index out of range
            sage: abs(g - B._get_answer()['gap'])   # tol 1e-22                         # needs cvxopt
            0.0
    ...
    cpdef slack(self, int i, sparse=False):
        ...
        EXAMPLES::
            ...
            -3.0
            sage: B = p.get_backend()                                                   # needs cvxopt
            sage: B1 = B.slack(1); B1               # tol 1e-08                         # needs cvxopt
            [0.0 0.0]
            [0.0 0.0]
            sage: B1.is_positive_definite()                                             # needs cvxopt
            True
            sage: x = sorted(p.get_values(x).values())                                  # needs cvxopt
            sage: x[0]*b1 + x[1]*b2 - b3 + B1       # tol 1e-09                         # needs cvxopt
            [0.0 0.0]
            [0.0 0.0]

        TESTS::

            sage: B.slack(7)                                                            # needs cvxopt
            Traceback (most recent call last):
            ...
            IndexError: list index out of range
