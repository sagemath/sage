...
cdef long get_ordp(x, PowComputer_class prime_pow) except? -10000:
    ...
    elif sage.rings.finite_rings.integer_mod.isinstance(x, IntegerMod_abstract):
...
cdef inline int cconv_shared(mpz_t out, x, long prec, long valshift, PowComputer_class prime_pow) except -2:
    ...
    elif isinstance(x, pAdicGenericElement) or sage.rings.finite_rings.integer_mod.isinstance(x, IntegerMod_abstract):
        x = x.lift()
