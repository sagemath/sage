...
class TateAlgebraIdeal(Ideal_generic):
    ...
    def _contains_(self, x):
        ...
        TESTS::
            ...
            sage: q1 = K(3^-6, 1)*x*y
            sage: q2 = K(3^2, 11)*x
            sage: q1.parent() is A and q2.parent() is A
            True
            sage: h = q1*f + q2*g
            sage: (h % I.groebner_basis()).is_zero()
            False
            sage: h in I
            True
        ...
        # Over the rational Tate algebra, membership is invariant under
        # multiplication by powers of the uniformizer.  If the direct reduction
        # lost too much precision after dividing by leading coefficients, test
        # membership through the integral model instead.
        Ao = A.integer_ring()
        gens = []
        try:
            for g in self.gens():
                if g:
                    ...
                    if v < 0:
                        g = g << (-v)
                gens.append(Ao(g))
            gb = Ao.ideal(gens).groebner_basis()
        except ValueError:
            return False
        if not gb:
            return False
        ...
        # If J is the integral ideal generated above, then J:saturation is
        # generated by the monic forms of the integral Groebner basis.  Hence
        # multiplying by pi^e, where e is the largest valuation of an element
        # of the basis, sends every saturated generator back into J.
        saturation_exponent = max(g.valuation() for g in gb)
        for n in range(start, start + saturation_exponent + 1):
            if (Ao(x << n) % gb).is_zero():
                return True
        return False
