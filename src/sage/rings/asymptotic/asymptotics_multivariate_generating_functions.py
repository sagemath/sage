...
class FractionWithFactoredDenominator(RingElement):
    ...
    def asymptotics(self, p, alpha, N, asy_var=None, numerical=0,
        ...
        i = d - 1
        while not all((X[i] * diff(h, X[i])).subs(p) for h, _ in df):
            i -= 1
        coordinate = i
    ...
    def asymptotics_smooth(self, p, alpha, N, asy_var, coordinate=None,
        ...
        if vector(p.values()) in QQ ** d:
            ...
            sP = [SR.var('p' + str(j)) for j in range(d)]
        ...
        while t in L:
            t = t + 't'
        T = [SR.var(t + str(i)) for i in range(d - 1)]
    ...
    def asymptotics_multiple(self, p, alpha, N, asy_var, coordinate=None,
        ...
        X = [SR(x) for x in R.gens()]
        G = SR(self.numerator())
        H = [SR(h) for h, _ in self.denominator_factored()]
        Hprod = prod(H)
        n = len(H)
        P = {SR(x): p[x] for x in R.gens()}
        ...
        while s in L:
            s = s + 't'
        S = [SR.var(s + str(i)) for i in range(n - 1)]
    ...
    def _crit_cone_combo(self, p, alpha, coordinate=None):
        ...
        V = [SR.var('sss' + str(i)) for i in range(n)]
        M = matrix(V) * Gamma
    ...
    def grads(self, p):
        ...
        X = R.gens()
        d = self.dimension()
        H = [h for h, _ in self.denominator_factored()]
        n = len(H)
    ...
    def singular_ideal(self):
        ...
        Hred = prod([h for h, _ in self.denominator_factored()])
    ...
    def relative_error(self, approx, alpha, interval, exp_scale=Integer(1),
        ...
        for r in interval:
            ...
            if mac[beta] == 0:
                stats_row.extend(None for _ in mac_approx[beta])
            else:
...
class FractionWithFactoredDenominatorSum(list):
    ...
    def sum(self):
        ...
        factors = []
        for f in self:
            factors.extend(q for q, _ in f.denominator_factored())
