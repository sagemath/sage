...
Note that ``I`` is not a symmetric Groebner basis::
    ...
    sage: G
    Symmetric Ideal (x_2 - x_1, x_1^2 + x_1) of Infinite polynomial ring in x over Rational Field
    sage: Q = R.quotient(G)
...
class SymmetricIdeal(Ideal_generic):
    ...
    The default ordering is lexicographic. We now compute a Groebner basis::
        ...
        [x_2*x_1*y_2 - x_2*x_1*y_1, x_1*y_2*y_1 + 2*x_1*y_2, x_2*y_2*y_1 + 2*x_2*y_1, x_2*x_1*y_1^2 + 2*x_2*x_1*y_1]
    ...
    def symmetrisation(self, N=None, tailreduce=False, report=None, use_full_group=False):
        ...
        EXAMPLES::
            ...
            Symmetric Ideal (-x_2^2, -x_1^2, x_2 + x_1) of Infinite polynomial ring in x over Rational Field
            sage: I.symmetrisation(N=3)                                                 # needs sage.combinat
            Symmetric Ideal (-x_2^2, -x_1^2, -2*x_1) of Infinite polynomial ring in x over Rational Field
            sage: I.symmetrisation(N=3, use_full_group=True)                            # needs sage.combinat
            Symmetric Ideal (-x_1^2, -2*x_1) of Infinite polynomial ring in x over Rational Field
        """
