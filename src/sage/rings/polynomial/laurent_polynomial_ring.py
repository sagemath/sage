...
class LaurentPolynomialRing_mpair(LaurentPolynomialRing_generic):
    ...
    def _random_bounded_monomial(self, min_valuation=-2, max_degree=2):
        """
        Computes a random monomial which has total degree at most
        ``max_degree``, total valuation at least ``min_valuation``.
        Also has degree and valuation within these bounds for each
        generator of the polynomial ring.

        .. NOTE::

            When both bounds are negative (positive) it is
            still possible for the monomial to have an exponent zero
            in one of the generators. As such, the degree (valuation)
            will be zero in this generator and the degree (valuation)
            will be outside of the user supplied bounds. The total
            degree or valuation is always respected though.
        ...
        EXAMPLES::
            ...
            (True, True, True)
            sage: tuple(f.valuation(x) >= -5 for x in R.gens())
            (True, True, True)

        ::

            sage: R.<x, y> = LaurentPolynomialRing(QQbar)
            ...
            (True, True)
            sage: tuple(f.valuation(x) >= 7 or f.valuation(x) == 0 for x in R.gens())
            (True, True)

        ::

            sage: R.<x0, x1, x2, x3> = LaurentPolynomialRing(GF(13^2))
            sage: f = R._random_bounded_monomial(-10, -8)
            sage: f.degree() <= -8
            True
            sage: f.valuation() >= -10
            True
            sage: tuple(f.degree(x) <= -8 or f.degree(x) == 0 for x in R.gens())
            (True, True, True, True)
            sage: tuple(f.valuation(x) >= -10 for x in R.gens())
            (True, True, True, True)
        ...
        TESTS::
            ...
            ....:         assert all(f.degree(x) <= high or f.degree(x) == 0 for x in R.gens())
            ....:         assert all(f.valuation(x) >= low or f.valuation(x) == 0 for x in R.gens())
        """
        ...
        for _ in range(self._n):
            # Set lower and upper bound from the sum of exp so far
