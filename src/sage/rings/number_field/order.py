...
class Order_absolute(Order):
    ...
    def __and__(left, right):
        ...
        We can also intersect orders with ideals

            sage: x = polygen(ZZ)
            sage: K.<a> = NumberField(x^3+x^2-2*x+8)
            sage: O = K.order(a)
            sage: I = O.ideal([2,2*a, a^2]); I
            Ideal (2, 2*a, a^2) of Order generated by a in Number Field in a with defining polynomial x^3 + x^2 - 2*x + 8
            sage: O.intersection(I)     # I is an ideal of O so the result should be just I
            Ideal (2, 2*a, a^2) of Order generated by a in Number Field in a with defining polynomial x^3 + x^2 - 2*x + 8
            sage: K.maximal_order().intersection(I)
            Ideal (2, a, a^2) of Maximal Order generated by [1/2*a^2 + 1/2*a, a^2] in Number Field in a with defining polynomial x^3 + x^2 - 2*x + 8
        ...
        if isinstance(right, NumberFieldOrderIdeal_generic) or isinstance(right, NumberFieldIdeal):
            ...
            # the result is then an ideal, not an order
