...
We compute a maximal order of a degree 10 field::
    ...
    Maximal Order in Number Field in a with defining polynomial x^10 + 10*x^9
     + 45*x^8 + 120*x^7 + 210*x^6 + 252*x^5 + 210*x^4 + 120*x^3 + 45*x^2 + 10*x + 18

We compute a suborder, which has index a power of 17 in the maximal order::

    sage: O = K.order(17*a); O
    Order in Number Field in a with defining polynomial x^10 + 10*x^9
     + 45*x^8 + 120*x^7 + 210*x^6 + 252*x^5 + 210*x^4 + 120*x^3 + 45*x^2 + 10*x + 18
...
class RelativeOrderFactory(OrderFactory):
    ...
    EXAMPLES::
        ...
        sage: L.order([i, j])
        Relative Order in Number Field in j
         with defining polynomial j^2 - 2 over its base field
    ...
    def _repr_(self):
        ...
        if self._is_maximal():
            ...
            if self.number_field().absolute_degree() == 2:
                f = (self.discriminant() / self.number_field().discriminant()).sqrt()
                s += f' of conductor {f}'
        gens = self.ring_generators()
        s += f' generated by {gens[0] if len(gens) == 1 else gens}'
        s += f' in {self._K}'
        return s
...
class Order_relative(Order):
    ...
    def absolute_order(self, names='z'):
        ...
        EXAMPLES::
            ...
            Relative Order in Number Field in i
             with defining polynomial x^2 + 1 over its base field
            sage: R.basis()
            [1, 6*i - g, -g*i + 2, 7*i - g]
        ...
        gamma.::
            ...
            Relative Order in Number Field in alpha0
