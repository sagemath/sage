...
class Order(Parent, sage.rings.abc.Order):
    ...
    def conductor(self, as_self_ideal=False):
        ...
        For orders in quadratic number fields, the method returns an integer.

            sage: K.<a> = QuadraticField(-101)
            sage: K.maximal_order().conductor()
            1
            sage: K.order(5*a).conductor()
            5
            sage: type(K.order(5*a).conductor())
            <class 'sage.rings.integer.Integer'>

        For orders in fields of degree larger than 2, we return an ideal of the ring of integers. This can be
        changed to an ideal of `self` by passing `as_self_ideal=True`.

            sage: K.<a> = CyclotomicField(7)
            sage: O = K.order(2*a)
            sage: O.conductor()     # returns an ideal of K.maximal_order()
            Fractional ideal (32)
            sage: O.conductor(as_self_ideal=True)   # returns an ideal of O
            Ideal (32, 64*a, 128*a^2, 256*a^3, 512*a^4, 1024*a^5) of Order generated by 2*a in Cyclotomic Field of order 7 and degree 6

        The conductor is no always principal.

            sage: K.<a> = NumberField(x^3-7)
            sage: p = K.ideal(3).factor()[0][0]
            sage: O = K.order((p**2).gens())
            sage: O.conductor().gens()
        ...
        # this was first implemented by @kevinywlui: https://github.com/sagemath/sage-archive-2023-02-01/commit/25b1b10776d43cd715e7b384d330b5330ad6069b
        O = self
