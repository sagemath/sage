...
class Order(Parent, sage.rings.abc.Order):
    ...
    def conductor(self, as_ideal=False):
        r"""
        Return the conductor of this order of a number field.

        The conductor is defined as `\mathfrak{f} = \{x \in \mathcal{O} \colon
        x\mathcal{O}_K \subseteq \mathcal{O}`. It is an ideal in `\mathcal{O}`.

        When this order is in a quadratic number field, it can be shown that
        `\mathfrak{f} = f\mathcal{O}_K`, where `f` is the unique positive
        integer such that the discriminant of this order is `f^2` times the
        discriminant of the containing quadratic field. In this case, we return
        the integer `f` instead unless specified.

        INPUT:

        - ``as_ideal`` -- boolean (default: ``False``); if ``False``, return the
          conductor as an integer when the number field is a quadratic field. If
          ``True``, always return the conductor as an ideal.

        OUTPUT: The conductor of this order, as an ideal or an integer.
        ...
        ::
            ...
            sage: O.conductor()
            10
            sage: c = O.conductor(as_ideal=True); c
            Ideal (5*t + 5, 10*t) of Order of conductor 10 generated by 5*t in Number Field in t with defining polynomial x^2 - 101 with t = 10.04987562112089?
            sage: c.free_module() == (10 * K).free_module()
            True
        ...
        ::
            ...
            sage: O = K.order(a)
            sage: O.conductor()
            Ideal (2*a^2 + 1, 2*a^2 + a, 3*a^2) of Order generated by a in Number Field in a with defining polynomial x^3 - 19

        We have not implemented conductor of relative extensions yet::

            sage: K = QQ[sqrt(2)][sqrt(3)]
            sage: O = K.order([1, K.0, K.1])
            sage: O.conductor()
            Traceback (most recent call last):
            ...
            NotImplementedError
        ...
        TESTS::
            ...
            sage: type(K.order(5*t).conductor(as_ideal=True))
            <class 'sage.rings.number_field.order_ideal.NumberFieldOrderIdeal_quadratic'>
            sage: K.<a> = NumberField(x^3 - 19)
            sage: type(K.order(a).conductor())
        ...
        if as_ideal or not isinstance(K, sage.rings.abc.NumberField_quadratic):
            # By definition, we want to find {a \in O | aOK <= O}
            # Let O = span_Z(f_i) and OK = span_Z(e_i)
            # Writing c = sum(c_i x^i), we want sum(c_i x^i e_j) in O for all j
            # Further writing x^i e_j = sum(d_ijk f_k), we have
            # sum(sum(c_i d_ijk over i) f_k over k) in O
            # <=> sum(c_i d_ijk over i) in Z
            # so we compute the (d_ijk)_k vectors, compute the Z-module they
            # generate, intersect it with Z^[K:Q] to get integer vectors, and
            # compute their coefficients in O
            OK = K.maximal_order()
            OM = self._module_rep

            rels = []
            for xi in K.gen(0).powers(K.degree()):
                cur_row = []
                for ej in OK.gens():
                    g = K(xi) * K(ej)
                    dij = OM.coordinates(g.list())
                    cur_row.extend(dij)
                rels.append(cur_row)
            ...
            # We keep track of the matrix as `span` puts basis into echelon form
            rels = Matrix(QQ, rels)
            rels_module = span(ZZ, rels)
            rels_integers = rels_module.intersection(ZZ**rels_module.degree())
            sols = span(ZZ, rels.solve_left(rels_integers.matrix()))

            return self.ideal(list(map(K, sols.gens())))
