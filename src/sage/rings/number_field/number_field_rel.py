...
class NumberField_relative(NumberField_generic):
    ...
    def composite_fields(self, other, names=None, both_maps=False, preserve_embedding=True):
        ...
        - ``both_maps`` -- boolean (default: ``False``);  if ``True``, return quadruples
        ...
        - ``preserve_embedding`` -- boolean (default: ``True``); has no effect, but is kept
    ...
    def order(self, *gens, **kwds):
        ...
        - ``check_is_integral`` -- boolean (default: ``True``), whether to check that each
          generator is integral.
        - ``check_rank`` -- boolean (default: ``True``), whether to check that the ring
          generated by ``gens`` is of full rank.
        - ``allow_subfield`` -- boolean (default: ``False``), if ``True`` and the generators
