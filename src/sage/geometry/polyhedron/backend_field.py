...
class Polyhedron_field(Polyhedron_base):
    ...
    Check that :trac:`19013` is fixed::
        ...
        sage: x = polygen(ZZ, 'x')
        sage: K.<phi> = NumberField(x^2-x-1, embedding=1.618)                           # needs sage.rings.number_field
        sage: P1 = Polyhedron([[0,1],[1,1],[1,-phi+1]])                                 # needs sage.rings.number_field
        sage: P2 = Polyhedron(ieqs=[[-1,-phi,0]])                                       # needs sage.rings.number_field
        sage: P1.intersection(P2)                                                       # needs sage.rings.number_field
