        ...
        class ParentMethods:
            def _test_supercommutativity(self, **options):
                ...
                ``self.some_elements()``::
                    ...
                    sage: E.<x,y,z> = ExteriorAlgebra(QQ)                               # needs sage.combinat sage.modules
                    sage: E._test_supercommutativity()                                  # needs sage.combinat sage.modules

                However, the elements tested can be customized with the
                ``elements`` keyword argument, but the elements must be
                homogeneous::

                    sage: E._test_supercommutativity(elements=[x+y, x*y-3*y*z, x*y*z])  # needs sage.combinat sage.modules
                    sage: E._test_supercommutativity(elements=[x+x*y])                  # needs sage.combinat sage.modules
