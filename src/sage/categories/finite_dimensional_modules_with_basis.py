    ...
    class ParentMethods:
        ...
        def annihilator(self, S, action=operator.mul, side='right', category=None):
            ...
            EXAMPLES::
                ...
                sage: F = FiniteDimensionalAlgebrasWithBasis(QQ).example(); F           # needs sage.modules
                An example of a finite dimensional algebra with basis:
                the path algebra of the Kronecker quiver
                (containing the arrows a:x->y and b:x->y) over Rational Field
                sage: x, y, a, b = F.basis()                                            # needs sage.modules
                sage: A = F.annihilator([a + 3*b + 2*y]); A                             # needs sage.modules
                Free module generated by {0} over Rational Field
                sage: [b.lift() for b in A.basis()]                                     # needs sage.modules
                [-1/2*a - 3/2*b + x]

            The category can be used to specify other properties of
            this subspace, like that this is a subalgebra::

                sage: center = F.annihilator(F.basis(), F.bracket,                      # needs sage.modules
                ....:                        category=Algebras(QQ).Subobjects())
                sage: (e,) = center.basis()                                             # needs sage.modules
                sage: e.lift()                                                          # needs sage.modules
                x + y
                sage: e * e == e                                                        # needs sage.modules
                True

            Taking annihilator is order reversing for inclusion::

                sage: A   = F.annihilator([]);    A  .rename("A")                       # needs sage.modules
                sage: Ax  = F.annihilator([x]);   Ax .rename("Ax")                      # needs sage.modules
                sage: Ay  = F.annihilator([y]);   Ay .rename("Ay")                      # needs sage.modules
                sage: Axy = F.annihilator([x,y]); Axy.rename("Axy")                     # needs sage.modules
                sage: P = Poset(([A, Ax, Ay, Axy], attrcall("is_submodule")))           # needs sage.combinat sage.graphs sage.modules
                sage: sorted(P.cover_relations(), key=str)                              # needs sage.combinat sage.graphs sage.modules
        ...
        def from_vector(self, vector, order=None, coerce=True):
            ...
            EXAMPLES::
                ...
                sage: p_mult = matrix([[0,0,0], [0,0,-1], [0,0,0]])                     # needs sage.modules
                sage: q_mult = matrix([[0,0,1], [0,0,0], [0,0,0]])                      # needs sage.modules
                sage: A = algebras.FiniteDimensional(                                   # needs sage.modules
                ....:         QQ, [p_mult, q_mult, matrix(QQ, 3, 3)], 'p,q,z')
                sage: A.from_vector(vector([1,0,2]))                                    # needs sage.modules
                p + 2*z
            """
            if order is None:
                try:
        ...
        def echelon_form(self, elements, row_reduced=False, order=None):
            ...
            EXAMPLES::
                ...
                sage: X = CombinatorialFreeModule(QQ, range(3), prefix="x")             # needs sage.modules
                sage: x = X.basis()                                                     # needs sage.modules
                sage: V = X.echelon_form([x[0]-x[1], x[0]-x[2], x[1]-x[2]]); V          # needs sage.modules
                [x[0] - x[2], x[1] - x[2]]
                sage: matrix(list(map(vector, V)))                                      # needs sage.modules
                [ 1  0 -1]
                [ 0  1 -1]

            ::

                sage: F = CombinatorialFreeModule(ZZ, [1,2,3,4])                        # needs sage.modules
                sage: B = F.basis()                                                     # needs sage.modules
                sage: elements = [B[1]-17*B[2]+6*B[3], B[1]-17*B[2]+B[4]]               # needs sage.modules
                sage: F.echelon_form(elements)                                          # needs sage.modules
        ...
        def invariant_module(self, S, action=operator.mul, action_on_basis=None,
            ...
            of the symmetric group::
                ...
                sage: M = FreeModule(ZZ, [1,2,3], prefix='M'); M.rename('M')            # needs sage.modules
            ...
            construct the `H`-invariant module of the group algebra `\QQ[G]`::
                ...
                sage: H = G.subgroup(G.gen(0))                                          # needs sage.groups
                sage: H                                                                 # needs sage.groups
                Subgroup generated by [(1,2,3,4)]
                 of (Dihedral group of order 8 as a permutation group)
                sage: H.cardinality()                                                   # needs sage.groups
                4
                sage: A = G.algebra(QQ)                                                 # needs sage.groups sage.modules
                sage: I = A.invariant_module(H)                                         # needs sage.groups sage.modules
                sage: [I.lift(b) for b in I.basis()]                                    # needs sage.groups sage.modules
                [() + (1,2,3,4) + (1,3)(2,4) + (1,4,3,2),
                 (2,4) + (1,2)(3,4) + (1,3) + (1,4)(2,3)]
                sage: all(h * I.lift(b) == I.lift(b)                                    # needs sage.groups sage.modules
    ...
    class ElementMethods:
        def dense_coefficient_list(self, order=None):
            ...
            EXAMPLES::
                ...
                sage: v = vector([0, -1, -3])                                           # needs sage.modules
                sage: v.dense_coefficient_list()                                        # needs sage.modules
                [0, -1, -3]
                sage: v.dense_coefficient_list([2,1,0])                                 # needs sage.modules
                [-3, -1, 0]
                sage: sorted(v.coefficients())                                          # needs sage.modules
                [-3, -1]
            """
            if order is None:
                try:
        ...
        def _vector_(self, order=None):
            ...
            EXAMPLES::
                ...
                sage: v = vector([0, -1, -3])                                           # needs sage.modules
                sage: v._vector_()                                                      # needs sage.modules
                (0, -1, -3)
                sage: C = CombinatorialFreeModule(QQ['x'], ['a','b','c'])               # needs sage.modules
                sage: C.an_element()._vector_()                                         # needs sage.modules
                (2, 2, 3)
            """
            if order is None:
    ...
    class MorphismMethods:
        def matrix(self, base_ring=None, side="left"):
            ...
            EXAMPLES::
                ...
                sage: X = CombinatorialFreeModule(ZZ, [1,2]); x = X.basis()             # needs sage.modules
                sage: Y = CombinatorialFreeModule(ZZ, [3,4]); y = Y.basis()             # needs sage.modules
                sage: phi = X.module_morphism(on_basis={1: y[3] + 3*y[4],               # needs sage.modules
                ....:                                   2: 2*y[3] + 5*y[4]}.__getitem__,
                ....:                         codomain=Y)
                sage: phi.matrix()                                                      # needs sage.modules
                [1 2]
                [3 5]
                sage: phi.matrix(side="right")                                          # needs sage.modules
                [1 3]
                [2 5]
            ...
            Check that :trac:`23216` is fixed::
                ...
                sage: X = CombinatorialFreeModule(QQ, [])                               # needs sage.modules
                sage: Y = CombinatorialFreeModule(QQ, [1,2,3])                          # needs sage.modules
                sage: Hom(X, Y).zero().matrix()                                         # needs sage.modules
                []
                sage: Hom(X, Y).zero().matrix().parent()                                # needs sage.modules
        ...
        def kernel(self):
            ...
            EXAMPLES::
                ...
                sage: SGA = SymmetricGroupAlgebra(QQ, 3)                                # needs sage.groups sage.modules
                sage: f = SGA.module_morphism(lambda x: SGA(x**2), codomain=SGA)        # needs sage.groups sage.modules
                sage: K = f.kernel()                                                    # needs sage.groups sage.modules
                sage: K                                                                 # needs sage.groups sage.modules
                Free module generated by {0, 1, 2} over Rational Field
                sage: K.ambient()                                                       # needs sage.groups sage.modules
