"""
Homomorphisms of Lie Algebras and Lie Superalgebras

AUTHORS:

- Travis Scrimshaw (07-15-2013): Initial implementation
- Eero Hakavuori (08-09-2018): Morphisms defined by a generating subset
- Travis Scrimshaw (02-26-2024): Adding Lie superalgebra support
"""
# ****************************************************************************
...
class LieAlgebraHomomorphism_im_gens(Morphism):
    r"""
    A homomorphism of Lie (super)algebras.

    Let `\mathfrak{g}` and `\mathfrak{g}^{\prime}` be Lie algebras.
    A linear map `f \colon \mathfrak{g} \to \mathfrak{g}^{\prime}` is a
    homomorphism (of Lie (super)algebras) if `f([x, y]) = [f(x), f(y)]` for
    all `x, y \in \mathfrak{g}`. Thus homomorphisms are completely determined
    by the image of the generators of `\mathfrak{g}`.

    INPUT:

    - ``parent`` -- a homset between two Lie (super)algebras
    ...
    def __init__(self, parent, im_gens, base_map=None, check=True):
        ...
        try:
            self._is_super = parent._is_super
        except AttributeError:  # coming from a more generic homset
            from sage.categories.lie_algebras import LieAlgebras
            self._is_super = parent.domain() in LieAlgebras.Super
        ...
        if check:
            if self._is_super:
                gens = parent.domain().super_lie_algebra_generators()
            else:
                gens = parent.domain().lie_algebra_generators()
            if len(im_gens) != len(gens):
                raise ValueError("number of images must equal number of generators")
            if (base_map is not None
                and not (base_map.domain() is parent.domain().base_ring()
                         and parent.codomain().base_ring().has_coerce_map_from(base_map.codomain())
                        )
               ):
    ...
    def _repr_type(self):
        ...
        TESTS::
            ...
            sage: phi = L.graded_morphism({x: x, y: y, z: z}, codomain=L)
            sage: phi._repr_type()
            'super Lie algebra'
        """
        if self._is_super:
            return "super Lie algebra"
        return "Lie algebra"
...
class LieAlgebraHomset(Homset):
    ...
    def __init__(self, X, Y, category=None, base=None, check=True):
        ...
        from sage.categories.lie_algebras import LieAlgebras
        SC = LieAlgebras(self.base()).Super()
        self._is_super = self.homset_category().is_subcategory(SC)
    ...
    def _repr_(self):
        ...
        """
        sup = "Super " if self._is_super else ""
        return "Set of {}Lie algebra morphisms from {} to {}".format(sup,
            ...
            super Lie algebra endomorphism of Super Lie algebra generated
             by (x, y, z) in degrees (1, 1, 2) over Rational Field
              Defn: x |--> x
                    ...
                    z |--> z
            sage: HS(HS.zero())
            Generic endomorphism of Super Lie algebra generated by (x, y, z)
             in degrees (1, 1, 2) over Rational Field
        """
        if isinstance(im_gens, Morphism):
            return im_gens
        from sage.categories.lie_algebras import LieAlgebras
        cat = LieAlgebras(self.base()).FiniteDimensional().WithBasis()
        if self._is_super:
            cat = cat.Super()
        if self.domain() in cat and self.codomain() in cat:
            try:
                return LieAlgebraMorphism_from_generators(im_gens, self.domain(),
...
class LieAlgebraMorphism_from_generators(LieAlgebraHomomorphism_im_gens):
    r"""
    A morphism between two Lie (super)algebras defined by images of a
    generating set as a Lie (super)algebra.

    This is the Lie (super)algebra morphism `\phi \colon L \to K` defined
    on the chosen basis of `L` to that of `K` be using the image of some
    ...
    def __init__(self, on_generators, domain=None, codomain=None, check=True, base_map=None, category=None):
        ...
        Failure of inferring codomain::
            ...
            Traceback (most recent call last):
            ...
            TypeError: codomain <class 'int'> is not a Lie (super)algebra
            ...
            Traceback (most recent call last):
            ...
            TypeError: domain Integer Ring is not a Lie (super)algebra
        ...
        We check that we can specify a base map to get a semi-linear
        ...
        if domain is None:
            ...
            if domain not in LieAlgebras and domain not in LieAlgebras.Super:
                raise TypeError("domain %s is not a Lie (super)algebra" % domain)
        if codomain is None:
            if not on_generators:
                raise ValueError("no elements to infer codomain from")
            codomain = cm.common_parent(*list(on_generators.values()))
            if codomain not in LieAlgebras and codomain not in LieAlgebras.Super:
                raise TypeError("codomain %s is not a Lie (super)algebra" % codomain)
        ...
        while True:
            ...
            for i,j in combinations(range(n), 2):
                ...
                Z = domain.bracket(domain.from_vector(spanning_set[i]),
                                   domain.from_vector(spanning_set[j]))
