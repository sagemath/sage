...
def _killing_equations(metric, chart=None, vector_name='X', latex_name=None,
    ...
    indices = tuple(dom.irange())

    raw_name = 'X' if vector_name is None else str(vector_name)
    prefix = _clean_killing_name(raw_name)
    effective_name = (
        'X' if vector_name is None or not any(ch.isalnum() for ch in raw_name)
        else raw_name
    )
    coord_names = [_clean_killing_name(c) for c in coords]
    xcomp = [function(f"{prefix}_{coord_names[p]}")(*coords)
             for p in range(n)]

    try:
        metric_dom = metric if dom == domain else metric.restrict(dom)
        gcomp = metric_dom.comp(frame)
        g = {(i, j): SR(gcomp[i, j, chart].expr(method='SR'))
    ...
    for p, i in enumerate(indices):
        for q in range(p, n):
            j = indices[q]
            expr = SR.zero()
            for r, k in enumerate(indices):
                Xk = xcomp[r]
                expr += Xk * g[(i, j)].diff(coords[r])
                expr += g[(k, j)] * Xk.diff(coords[p])
                expr += g[(i, k)] * Xk.diff(coords[q])
            expr = chart.simplify(expr, method='SR')
            eqs.append(expr == 0)

    if not return_vector:
        return eqs

    X = dom.vector_field(*xcomp, frame=frame, chart=chart,
                         name=effective_name, latex_name=latex_name)
    return eqs, X
    ...
    def inverse(self, expansion_symbol=None, order=1):
        ...
        if expansion_symbol is not None:
            if self._inverse is not None and bool(self._inverse._components):
                comp = next(iter(self._inverse._components.values()))
                si = self._fmodule._sindex
                try:
                    entry = comp[si, si]
                except (KeyError, ValueError, TypeError):
                    entry = None
                if (getattr(entry, '_expansion_symbol', None) == expansion_symbol
                    and getattr(entry, '_order', None) == order):
                    return self._inverse
