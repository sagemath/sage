...
class Function_zeta(GinacFunction):
    def __init__(self):
        ...
        EXAMPLES::
            ...
            sage: zeta(x)                                                               # optional - sage.symbolic
            zeta(x)
            ...
            zeta(I)
            sage: zeta(I).n()                                                           # optional - sage.symbolic
            0.00330022368532410 - 0.418155449141322*I
            sage: zeta(sqrt(2))                                                         # optional - sage.symbolic
            zeta(sqrt(2))
            sage: zeta(sqrt(2)).n()  # rel tol 1e-10                                    # optional - sage.symbolic
            3.02073767948603

        It is possible to use the ``hold`` argument to prevent
        automatic evaluation::

            sage: zeta(2, hold=True)                                                    # optional - sage.symbolic
            zeta(2)

        To then evaluate again, we currently must use Maxima via
        :meth:`sage.symbolic.expression.Expression.simplify`::

            sage: a = zeta(2, hold=True); a.simplify()                                  # optional - sage.symbolic
            1/6*pi^2
        ...
        :wikipedia:`Stieltjes constants <Stieltjes_constants>`::
            ...
            sage: s = SR('s')                                                           # optional - sage.symbolic
            sage: zeta(s).series(s==1, 2)                                               # optional - sage.symbolic
        ...
        expansion of `\zeta`-type singularities::
            ...
            sage: zeta(2*s/(s+1)).series(s==1, 2)                                       # optional - sage.symbolic
            2*(s - 1)^(-1) + (euler_gamma + 1) + (-1/2*stieltjes(1))*(s - 1) + Order((s - 1)^2)


        TESTS::

            sage: latex(zeta(x))                                                        # optional - sage.symbolic
            \zeta(x)
            sage: a = loads(dumps(zeta(x)))                                             # optional - sage.symbolic
            sage: a.operator() == zeta                                                  # optional - sage.symbolic
            True
            sage: zeta(x)._sympy_()                                                     # optional - sympy sage.symbolic
            zeta(x)

            sage: zeta(1)                                                               # optional - sage.symbolic
            Infinity
            sage: zeta(x).subs(x=1)                                                     # optional - sage.symbolic
            Infinity

        Check that :trac:`19799` is resolved::

            sage: zeta(pi)                                                              # optional - sage.symbolic
            zeta(pi)
            sage: zeta(pi).n()  # rel tol 1e-10                                         # optional - sage.symbolic
            1.17624173838258

        Check that :trac:`20082` is fixed::

            sage: zeta(x).series(x==pi, 2)                                              # optional - sage.symbolic
            (zeta(pi)) + (zetaderiv(1, pi))*(-pi + x) + Order((pi - x)^2)
            sage: (zeta(x) * 1/(1 - exp(-x))).residue(x==2*pi*I)                        # optional - sage.symbolic
            zeta(2*I*pi)

        Check that :trac:`20102` is fixed::

            sage: (zeta(x)^2).series(x==1, 1)                                           # optional - sage.symbolic
            1*(x - 1)^(-2) + (2*euler_gamma)*(x - 1)^(-1)
            + (euler_gamma^2 - 2*stieltjes(1)) + Order(x - 1)
            sage: (zeta(x)^4).residue(x==1)                                             # optional - sage.symbolic
        ...
        Check that the right infinities are returned (:trac:`19439`)::
            ...
            sage: zeta(1.0)
            +infinity
            sage: zeta(SR(1.0))                                                         # optional - sage.symbolic
            Infinity

        Fixed conversion::

            sage: zeta(3)._maple_init_()                                                # optional - sage.symbolic
            'Zeta(3)'
            sage: zeta(3)._maple_().sage()  # optional - maple                          # optional - sage.symbolic
            zeta(3)
        """
...
class Function_stieltjes(GinacFunction):
    def __init__(self):
        ...
        EXAMPLES::
            ...
            sage: _ = var('n')                                                          # optional - sage.symbolic
            sage: stieltjes(n)                                                          # optional - sage.symbolic
            stieltjes(n)
            sage: stieltjes(0)                                                          # optional - sage.symbolic
            euler_gamma
            sage: stieltjes(2)                                                          # optional - sage.symbolic
            stieltjes(2)
            sage: stieltjes(int(2))                                                     # optional - sage.symbolic
            stieltjes(2)
        ...
        automatic evaluation::
            ...
            sage: stieltjes(0, hold=True)                                               # optional - sage.symbolic
            stieltjes(0)

            sage: latex(stieltjes(n))                                                   # optional - sage.symbolic
            \gamma_{n}
            sage: a = loads(dumps(stieltjes(n)))                                        # optional - sage.symbolic
            sage: a.operator() == stieltjes                                             # optional - sage.symbolic
            True
            sage: stieltjes(x)._sympy_()                                                # optional - sympy sage.symbolic
            stieltjes(x)

            sage: stieltjes(x).subs(x==0)                                               # optional - sage.symbolic
            euler_gamma
        """
...
class Function_HurwitzZeta(BuiltinFunction):
    def __init__(self):
        ...
        TESTS::
            ...
            sage: latex(hurwitz_zeta(x, 2))                                             # optional - sage.symbolic
            \zeta\left(x, 2\right)
            sage: hurwitz_zeta(x, 2)._sympy_()                                          # optional - sympy sage.symbolic
            zeta(x, 2)
        """
    ...
    def _eval_(self, s, x):
        ...
        TESTS::
            ...
            sage: hurwitz_zeta(x, 1)                                                    # optional - sage.symbolic
            zeta(x)
            sage: hurwitz_zeta(4, 3)                                                    # optional - sage.symbolic
            1/90*pi^4 - 17/16
            sage: hurwitz_zeta(-4, x)                                                   # optional - sage.symbolic
            -1/5*x^5 + 1/2*x^4 - 1/3*x^3 + 1/30*x
            sage: hurwitz_zeta(3, 0.5)
            8.41439832211716
            sage: hurwitz_zeta(0, x)                                                    # optional - sage.symbolic
            -x + 1/2
        """
        if x == 1:
            return zeta(s)
    ...
    def _evalf_(self, s, x, parent=None, algorithm=None):
        ...
        TESTS::
            ...
            sage: hurwitz_zeta(11/10, 1/2).n()                                          # optional - sage.symbolic
            12.1038134956837
            sage: hurwitz_zeta(11/10, 1/2).n(100)                                       # optional - sage.symbolic
    ...
    def _derivative_(self, s, x, diff_param):
        ...
        TESTS::
            ...
            sage: diff(hurwitz_zeta(x, y), y)                                           # optional - sage.symbolic
...
def hurwitz_zeta(s, x, **kwargs):
    ...
    Symbolic evaluations::
        ...
        sage: hurwitz_zeta(-4, x)                                                       # optional - sage.symbolic
        -1/5*x^5 + 1/2*x^4 - 1/3*x^3 + 1/30*x
        sage: hurwitz_zeta(7, -1/2)                                                     # optional - sage.symbolic
        127*zeta(7) - 128
        sage: hurwitz_zeta(-3, 1)                                                       # optional - sage.symbolic
        1/120

    Numerical evaluations::

        sage: hurwitz_zeta(3, 1/2).n()                                                  # optional - sage.symbolic
        8.41439832211716
        sage: hurwitz_zeta(11/10, 1/2).n()                                              # optional - sage.symbolic
        12.1038134956837
        sage: hurwitz_zeta(3, x).series(x, 60).subs(x=0.5).n()                          # optional - sage.symbolic
        8.41439832211716
        sage: hurwitz_zeta(3, 0.5)
        8.41439832211716
...
class Function_zetaderiv(GinacFunction):
    def __init__(self):
        ...
        EXAMPLES::
            ...
            sage: zetaderiv(1, x)                                                       # optional - sage.symbolic
            zetaderiv(1, x)
            sage: zetaderiv(1, x).diff(x)                                               # optional - sage.symbolic
            zetaderiv(2, x)
            sage: var('n')                                                              # optional - sage.symbolic
            n
            sage: zetaderiv(n, x)                                                       # optional - sage.symbolic
            zetaderiv(n, x)
            sage: zetaderiv(1, 4).n()                                                   # optional - sage.symbolic
            -0.0689112658961254
            sage: import sage.libs.mpmath; sage.libs.mpmath.all.diff(lambda x: sage.libs.mpmath.all.zeta(x), 4)               # optional - mpmath
            mpf('-0.068911265896125382')

        TESTS::

            sage: latex(zetaderiv(2, x))                                                # optional - sage.symbolic
            \zeta^\prime\left(2, x\right)
            sage: a = loads(dumps(zetaderiv(2, x)))                                     # optional - sage.symbolic
            sage: a.operator() == zetaderiv                                             # optional - sage.symbolic
            True

            sage: b = RBF(3/2, 1e-10)                                                   # optional - sage.libs.flint
            sage: zetaderiv(1, b, hold=True)                                            # optional - sage.libs.flint sage.symbolic
            zetaderiv(1, [1.500000000 +/- 1.01e-10])
            sage: zetaderiv(b, 1)                                                       # optional - sage.libs.flint sage.symbolic
    ...
    def _evalf_(self, n, x, parent=None, algorithm=None):
        ...
        TESTS::
            ...
            sage: zetaderiv(0, 3, hold=True).n() == zeta(3).n()                         # optional - sage.symbolic
            True
            sage: zetaderiv(2, 3 + I).n()                                               # optional - sage.symbolic
    ...
    def _method_arguments(self, k, x, **args):
        ...
        TESTS::
            ...
            sage: zetaderiv(1, RBF(3/2, 0.0001))                                        # optional - sage.libs.flint
            [-3.93 +/- ...e-3]
        """
        return [x, k]
...
class DickmanRho(BuiltinFunction):
    ...
    EXAMPLES::
        ...
        sage: plot(log(dickman_rho(x)), (x, 0, 15))                                     # optional - sage.plot sage.symbolic
    ...
    def __init__(self):
        ...
        TESTS::
            ...
            sage: dickman_rho(x)                                                        # optional - sage.symbolic
            dickman_rho(x)
            sage: dickman_rho(3)
            0.0486083882911316
            sage: dickman_rho(pi)                                                       # optional - sage.symbolic
            0.0359690758968463
        """
        self._cur_prec = 0
