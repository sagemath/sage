...
class PermutationGroup_generic(FiniteGroup):
    ...
    def _table_of_marks(self):
        ...
        The table of marks of a finite group `G` records, for each pair
        of conjugacy classes of subgroups of `G`, the number of fixed
        points of one subgroup acting on the cosets of the other.  It is
        returned as a GAP object, so that the usual GAP functions such
        as ``MarksTom``, ``OrdersTom`` and ``RepresentativeTom`` can be
        applied to it.

        Subclasses for groups whose table of marks is available in GAP's
        ``tomlib`` package may override this to avoid computing the
        table from the group.

        EXAMPLES::

            sage: G = PermutationGroup([[(1,2,3)], [(1,2)]])
            sage: G._table_of_marks()
            TableOfMarks( Sym( [ 1 .. 3 ] ) )

        TESTS::

            sage: G._table_of_marks() is G._table_of_marks()
            True
        """
        return libgap.TableOfMarks(self.gap())
