...
def is_groupbygens(group, gens):
    """
    Check if a ``group`` is generated by given ``gens``.
    ...
    if not isinstance(gens, (list, tuple)):
        raise ValueError("gens must be a list or tuple.")
    return set(group.AsList()) == set(libgap.GroupByGenerators(gens).AsList())


@cached_method
def minimum_generating_set(group, gap_based=False) -> list:
    """
    ...
    if not isinstance(group, GapElement):
        converter = group._element_constructor_
        group = group._libgap_()
    elif not gap_based:
