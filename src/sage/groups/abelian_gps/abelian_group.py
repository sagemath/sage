...
class AbelianGroup_class(UniqueRepresentation, AbelianGroupBase):
    ...
    def subgroups(self, check=False):
        ...
        Check that :issue:`36417` is fixed::

            sage: # needs sage.libs.gap sage.rings.number_field
            sage: x = polygen(QQ)
            sage: K.<a> = NumberField(x^2 + 21)
            sage: clG = K.class_group()
            sage: clG.gens_orders()
            (2, 2)
            sage: sorted([s.order() for s in clG.subgroups()])
            [1, 2, 2, 2, 4]
        """
        if not self.is_finite():
            raise ValueError("group must be finite")
    ...
    def subgroup_reduced(self, elts, verbose=False):
        ...
        sub_gens = [
            prod([gens[i] ** (x[0][i] % orders[i] if orders[i] else x[0][i])
                  for i in range(d)], self.one())
            for x in new_basis if x[1] != 1
        ]
        return self.subgroup(sub_gens)
