SAGE_SPKG_CONFIGURE([meson], [dnl
    dnl scipy 1.15.2 needs meson >= 1.5.0; 1.11.1 is broken on macOS
    AC_CACHE_CHECK([for meson >= 1.11.2], [ac_cv_path_MESON], [dnl
            ...
            AC_PATH_PROGS_FEATURE_CHECK([MESON], [meson], [dnl
                AS_IF([meson_version=$($ac_path_MESON --version 2>&1)], [dnl
                    AS_IF([test -n "$meson_version"], [dnl
                        AX_COMPARE_VERSION([$meson_version], [ge], [1.11.2], [dnl
