...
sage_build_cython.built_distributions = ['sagemath-ntl']
...
python_packages, python_modules, cython_modules = find_python_sources(
    '.', ['sage'], distributions=['sagemath-ntl'])
extra_files = find_extra_files(
    '.', ['sage'], '/doesnotexist', distributions=['sagemath-ntl'])
package_data = {"": [f
                     for pkg, files in extra_files.items()
                     for f in files ]}
package_data.update({'sage.libs.ntl': ['*.h', '*.pxi']})
