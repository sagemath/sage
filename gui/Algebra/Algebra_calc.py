...
class Algebra_calc(QWidget):
    def __init__(self, parent=None):
        ...
        # Result label for roots
        self.result_label = QLabel("Roots: ---")
        main_layout.addWidget(self.result_label)

        # --- Input for x0 to compute quadratic slope ---
        self.x0_label = QLabel("x₀ (point to find slope of ax²+bx+c):")
        self.x0_input = QLineEdit()
        x0_row = QHBoxLayout()
        x0_row.addWidget(self.x0_label)
        x0_row.addWidget(self.x0_input)
        main_layout.addLayout(x0_row)

        # Button for quadratic slope at x0
        self.compute_button_slope_quad = QPushButton("Compute Slope of Quadratic")
        self.compute_button_slope_quad.clicked.connect(self.compute_quad_slope)
        main_layout.addWidget(self.compute_button_slope_quad)

        # Result label for quadratic slope
        self.result_label_quad_slope = QLabel("Slope at x₀: ---")
        main_layout.addWidget(self.result_label_quad_slope)
        ...
        # Button to compute slope between points (unchanged)
        self.compute_slope_button = QPushButton("Compute Slope")
        self.compute_slope_button.clicked.connect(self.compute_slope)
        main_layout.addWidget(self.compute_slope_button)
    ...
    def compute_quad_slope(self):
        try:
            a = float(self.a_input.text())
            b = float(self.b_input.text())
            x0 = float(self.x0_input.text())
            slope = 2*a*x0 + b
            self.result_label_quad_slope.setText(f"Slope at x₀ = {x0:.4f}: {slope:.4f}")
        except ValueError:
            QMessageBox.warning(self, "Input Error", "Please enter valid numbers for coefficients and x₀.")
