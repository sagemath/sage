...
EOF
                SET_SOURCES+="((yes | unminimize) || echo \"(ignored)\") && "
                if [ -n "$DIST_UPGRADE" ]; then
                    SET_SOURCES+="(sed -i.bak $DIST_UPGRADE /etc/apt/sources.list && apt-get update && DEBIAN_FRONTEND=noninteractive apt-get -y dist-upgrade) && "
                fi
                if [ -n "$EXTRA_REPOSITORIES" ]; then
                    SET_SOURCES+="($UPDATE $INSTALL software-properties-common && ($INSTALL gpg gpg-agent || echo \"(ignored)\")) && "
                    for repo in $EXTRA_REPOSITORIES; do
                        SET_SOURCES+="$SUDO add-apt-repository $repo && "
                    done
                fi
        esac
        ;;
...
EOF
  case "$IGNORE_MISSING_SYSTEM_PACKAGES" in
    no)
        cat <<EOF
RUN $SET_SOURCES $UPDATE $INSTALL $SYSTEM_PACKAGES $CLEAN
EOF
        ;;
    yes)
        if [ -n "$EXISTS" ]; then
            ...
            cat <<EOF
RUN $SET_SOURCES $UPDATE EXISTING_PACKAGES=""; for pkg in $SYSTEM_PACKAGES; do echo -n .; if $EXISTS \$pkg; then EXISTING_PACKAGES="\$EXISTING_PACKAGES \$pkg"; echo -n "\$pkg"; fi; done; $INSTALL \$EXISTING_PACKAGES || (echo "Trying again one by one:"; for pkg in \$EXISTING_PACKAGES; do echo "Trying to install \$pkg"; $INSTALL \$pkg || echo "(ignoring error)"; done); : $CLEAN
EOF
        else
            # Try in one shot, fall back to one by one.  Separate "RUN" commands
            # for caching by docker.
            cat <<EOF
RUN $SET_SOURCES $UPDATE $INSTALL $SYSTEM_PACKAGES || echo "(ignoring error)"
EOF
